...
class DriftPhotoRepository implements PhotoRepository {
  ...
  Stream<List<PhotoEntry>> watchAll(String uid) {
    final query = _db.select(_db.photoEntries)
  ...
  @override
  Future<List<PhotoEntry>> getDirtyEntries(String uid) async {
    final query = _db.select(_db.photoEntries)
  ...
  @override
  Future<void> upsert(PhotoEntry entry, {bool isDirty = true}) async {
    await _db.into(_db.photoEntries).insertOnConflictUpdate(
          photoEntryToCompanion(entry, isDirty: isDirty),
        );
  }

  @override
  Future<void> markSynced(String uid, List<String> ids) async {
    await (_db.update(_db.photoEntries)
          ..where((tbl) => tbl.uid.equals(uid) & tbl.id.isIn(ids)))
        .write(const PhotoEntriesCompanion(isDirty: Value(false)));
  }
  ...
  Future<void> hardDeleteAll(String uid) async {
    await (_db.delete(_db.photoEntries)..where((tbl) => tbl.uid.equals(uid))).go();
  }
}
